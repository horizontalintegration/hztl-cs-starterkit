...
export async function createManagementClient() {
  ...
  try {
    const client = contentstackManagementSDK.client({
      region: process.env.NEXT_PUBLIC_CONTENTSTACK_REGION || 'AZURE_NA',
    });
    ...
    throw new Error(
      `Failed to initialize Contentstack Management client: ${
...
export async function testConnection(): Promise<boolean> {
  try {
    ...
    throw new Error(
      `Contentstack Management API connection test failed: ${
        error instanceof Error ? error.message : 'Unknown error'
      }`
    );
  }
